...
This directory (`evaluation_framework/data/`) is designed to be **standalone**. Annotation files in `data/annotations/` are independent copies that the evaluation framework maintains for reproducibility, separate from `paper/benchmark/data/annotations/`.

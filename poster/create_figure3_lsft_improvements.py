...
def load_single_cell_lsft():
    """Load single-cell LSFT improvements."""
    single_cell_lsft = {
        'Adamson': {
    ...
    # Load K562 data
    k562_scgpt = pd.read_csv(project_root / 'results/single_cell_analysis/k562/lsft/lsft_single_cell_summary_k562_lpm_scgptGeneEmb.csv')
    k562_scfoundation = pd.read_csv(project_root / 'results/single_cell_analysis/k562/lsft/lsft_single_cell_summary_k562_lpm_scFoundationGeneEmb.csv')
    
    single_cell_lsft['K562']['scGPT'] = k562_scgpt[k562_scgpt['top_pct'] == 0.05]['pert_mean_delta_r'].values[0]
    single_cell_lsft['K562']['scFoundation'] = k562_scfoundation[k562_scfoundation['top_pct'] == 0.05]['pert_mean_delta_r'].values[0]
    
    # Try to load RPE1 LSFT data if available
    rpe1_lsft_dir = project_root / 'results/single_cell_analysis/rpe1/lsft'
    if rpe1_lsft_dir.exists():
        for baseline_file in rpe1_lsft_dir.glob('lsft_single_cell_summary_rpe1_*.csv'):
            baseline_name = baseline_file.stem.replace('lsft_single_cell_summary_rpe1_', '')
            df = pd.read_csv(baseline_file)
            if len(df) > 0:
                delta_r = df[df['top_pct'] == 0.05]['pert_mean_delta_r'].values[0]
                if baseline_name == 'lpm_selftrained':
                    single_cell_lsft['RPE1']['Self-trained PCA'] = delta_r
                elif baseline_name == 'lpm_scgptGeneEmb':
                    single_cell_lsft['RPE1']['scGPT'] = delta_r
                elif baseline_name == 'lpm_scFoundationGeneEmb':
                    single_cell_lsft['RPE1']['scFoundation'] = delta_r
                elif baseline_name == 'lpm_gearsPertEmb':
                    single_cell_lsft['RPE1']['GEARS'] = delta_r
                elif baseline_name == 'lpm_randomGeneEmb':
                    single_cell_lsft['RPE1']['Random Gene'] = delta_r
                elif baseline_name == 'lpm_randomPertEmb':
                    single_cell_lsft['RPE1']['Random Pert'] = delta_r
...
if __name__ == "__main__":
    ...
    print("\nKey finding: LSFT provides large gains for weak embeddings,")
    print("but tiny (or negative) gains for self-trained PCA.")
